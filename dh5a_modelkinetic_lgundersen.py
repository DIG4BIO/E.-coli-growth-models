# -*- coding: utf-8 -*-
"""DH5a_ModelKinetic_LGundersen.ipynb

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/1aWlMjZ7IyB_lmmTSTbz-a420qbRFMFY2
...
Lopes_data = pd.read_excel('/content/drive/MyDrive/Data_Org.xlsx', sheet_name='DB20Ua')
...
#WHOLE DATA
IST_s_data = pd.read_excel('/content/drive/MyDrive/Data_Org.xlsx', sheet_name='DSF20Ua')
...
plt.bar(output_labels, scale_factors_d, color='red')
...
plt.bar(sorted_global.index, sorted_global.values, color='red')
...
for i, output in enumerate(output_labels):
    ...
    ax = axes[i]
    ax.bar(sorted_delta.index, sorted_delta.values, color='red')
...
filtered_subsets = gamma_df[(gamma_df["gamma_K"] < 10) & (gamma_df["subset_size"] > 2)]
...
for i in range(4):
    key = keys[i]
    axs[i].plot(t_exp, Y_d[i, :], 'o', label='Experimental', color='red')
...
for i, (var, idx) in enumerate(output_indices.items()):
    var_data = Y_cov[:, :, idx]
    for y in var_data:
        axs[i].plot(t_d, y, color='red', alpha=0.1)
    ...
    axs[i].fill_between(t_d, p10, p90, alpha=0.1, color='red', label='80% band')
...
for i, (var, idx) in enumerate(output_indices.items()):
    ...
    for y in var_data:
        axs[i].plot(t_d, y, color='red', alpha=0.1, linewidth=0.8)
...
for i, var in enumerate(["X", "S", "P", "A"]):
    ...
    point_colors = [
        'blue' if abs(mu[j]) <= 2 * sigma[j] / np.sqrt(r) else 'red'
...
for i in range(4):
    axs[i].plot(t_t2, Y_t2[i], 'o', label='Experimental', color='darkorange', markersize=10)
...
legend1 = fig.legend(var_lines, [l.get_label() for l in var_lines],
                     loc='lower center', bbox_to_anchor=(0.5, 0.06), ncol=4, fontsize=14, title="Variables", title_fontsize=14)

legend2 = fig.legend(style_lines, [l.get_label() for l in style_lines],
                     loc='lower center', bbox_to_anchor=(0.5, -0.01), ncol=4, fontsize=14, title="Simulation Type", title_fontsize=14)
...
for i, (var, idx) in enumerate(output_indices.items()):
    ...
    # SIMULATION SHAKE FLASK
    ax.plot(t_d, y_sim_t2_final_T[idx], color='darkorange', linewidth=2, linestyle='-')
    # SIMULATION BATCH
    #ax.plot(t_d, y_sim_t_final_T[idx], linestyle="-", color='red', linewidth=2)
    # SIMULATION ESTIMATION
    ax.plot(t_d, y_sim_d_final_T[idx], linestyle="-", color='black', linewidth=2)
...
style_lines = [
    mlines.Line2D([], [], color='black', linestyle='-', label='Simulation Estimate'),
    mlines.Line2D([], [], color='darkorange', linestyle='-', label='Simulation Shake Flask')
    #mlines.Line2D([], [], color='red', linestyle='-', label='Simulation Batch'),
]
...
for i, (var, idx) in enumerate(output_indices.items()):
    ...
    # SIMULATION SHAKE FLASK
    ax.plot(t, y_sim_10_T[idx], color='darkorange', linewidth=2, linestyle='-')
    # SIMULATION BATCH
    ax.plot(t, y_sim_20_T[idx], color='red', linewidth=2, linestyle='-')
    # SIMULATION ESTIMATION
    ax.plot(t, y_sim_30_T[idx], color='black', linewidth=2, linestyle='-')

    ax.plot(t, y_sim_205_T[idx], color='red', linewidth=2, linestyle='dashdot')
    ax.plot(t, y_sim_201_T[idx], color='red', linewidth=2, linestyle='--')
...
style_lines = [
    mlines.Line2D([], [], color='darkorange', linestyle='-', label='10 g/L'),
    mlines.Line2D([], [], color='red', linestyle='-', label='20 g/L'),
    mlines.Line2D([], [], color='black', linestyle='-', label='30 g/L'),
    mlines.Line2D([], [], color='red', linestyle=':', label='0.5 g/L'),
    mlines.Line2D([], [], color='red', linestyle='--', label='0.01 g/L'),
]
...
legend2 = fig.legend(style_lines, [l.get_label() for l in style_lines],
                     loc='lower center', bbox_to_anchor=(0.5, 0.05), ncol=5, fontsize=11)
